...
class MainActivity : AppCompatActivity() {
    ...
    private class Demo(
        ...
        val vol: Double = 0.55,
        /** Velocity autocorrelation — higher = longer, smoother trends. */
    ...
    // AR(1) velocities: the walk integrates a slowly-decaying velocity, so it
    // trends in a direction for a stretch (with small counter-ticks) rather than
    // jittering up/down each tick. See the RN example's feeds.ts for the rationale.
    ...
    /** Advance `candlePrice` one trending step and return it. */
    private fun candleStep(): Double {
        val m = 0.93; val kick = 1.4 * sqrt(1 - m * m)
        candleVel = candleVel * m + rng.nextGaussian() * kick
        candlePrice += candleVel + (150.0 - candlePrice) * 0.006
        return candlePrice
    }
    ...
    private fun selectDemo(index: Int) {
        ...
        v = demo.center; velocity = 0.0; spike = 0.0; ticks = 0; elapsedMs = 0
        val n = 150; val span = demo.window
        val seed = (0 until n).map { i ->
            LivelinePoint(now - span + i / (n - 1.0) * span, trendStep(demo.center, demo.vol, demo.momentum, demo.reversion))
        }
        chart.setData(seed)
    ...
    private fun seedCandles() {
        ...
        candlePrice = 150.0; candleVel = 0.0
        val nowSec = System.currentTimeMillis() / 1000.0
        val liveBucket = floor(nowSec / candleWidth) * candleWidth
        for (i in 0 until 18) {
            val open = candlePrice; var hi = open; var lo = open; var close = open
            repeat(10) { close = candleStep(); hi = max(hi, close); lo = min(lo, close) }
            candleList.add(LivelineCandle(liveBucket - candleWidth * (18 - i), open, hi, lo, close))
        }
    ...
    private fun startFeed(demo: Demo) {
        handler.post(object : Runnable {
            override fun run() {
                ...
                when (demo.kind) {
                    Kind.SPIKES -> {
                        // Trending idle baseline (~14) with occasional decaying spikes.
                        val base = trendStep(demo.center, 1.2, 0.85, 0.06)
                        if (Random.nextDouble() > 0.985) spike = Random.nextDouble(40.0, 75.0)
                        spike *= 0.82
                        chart.push(LivelinePoint(System.currentTimeMillis() / 1000.0, (base + spike).coerceIn(2.0, 100.0)))
                    }
                    ...
                    Kind.ORDERBOOK -> {
                        trendStep(demo.center, demo.vol, demo.momentum, demo.reversion)
